...
function updateOrderStatus(selectElement) {
	const storeSeq = selectElement.dataset.orderId;
    const newStatus = selectElement.value;
	const currentStatus = selectElement.querySelector('option:checked').value;
	
	console.log("storeSeq:", storeSeq); // 디버깅용
    console.log("newStatus:", newStatus); // 디버깅용
	
	if (currentStatus === 'COMPLETED') {
	    swal.fire({
	        icon: 'info',
	        title: '변경 불가',
	        text: '완료된 주문은 상태를 변경할 수 없습니다.'
	    });
	...
	swal.fire({
		...
		if(result.isConfirmed) {
			$.ajax({
	               ...
	               success: function(response) {
	                   swal.fire('완료', '주문 상태가 수정되었습니다.', 'success');
	               },
	               error: function(xhr, status, error) {
