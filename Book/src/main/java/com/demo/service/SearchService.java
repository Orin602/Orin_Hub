...
//    }
	...
	public ItemDTO getBookByDocId(String docId) {
		try {
			String url = String.format(
					"https://www.nl.go.kr/NL/search/openApi/search.do?key=%s&detailSearch=true&docid=%s&apiType=json",
					apiKey, docId);
			...
			if (response != null && !response.isEmpty()) {
				...
				if (resultNode.isArray() && resultNode.size() > 0) {
					JsonNode itemNode = resultNode.get(0);
					ItemDTO item = new ItemDTO();
					item.setTitleInfo(itemNode.path("titleInfo").asText().replaceAll("<[^>]*>", "").trim());
					item.setAuthorInfo(itemNode.path("authorInfo").asText().replaceAll("<[^>]*>", "").trim());
					String imageUrl = itemNode.path("imageUrl").asText().trim();
					item.setImageUrl(imageUrl.isEmpty() ? "/images/no_img.jpg" : "http://cover.nl.go.kr/" + imageUrl);
					item.setKdcName1s(itemNode.path("kdcName1s").asText().trim());
					item.setDescription(itemNode.path("publisher").asText("")); // 설명 대신 출판사 정보 넣기 (필요시)
					return item;
				}
			}
		} catch (Exception e) {
